...
async def test_user_cannot_delete_other_users_inverter(client, db_session):
    ...
    from tests.helpers import create_inverter_in_db, create_user_in_db

    # Create two users
    await create_user_in_db(db_session, email="user_a@example.com", first_name="User", last_name="A")

    user_b = await create_user_in_db(db_session, email="user_b@example.com", first_name="User", last_name="B")

    # Create inverter for User B
    inverter_b = await create_inverter_in_db(db_session, user_id=user_b.id, serial_logger="USER-B-INVERTER")

    # Login as User A
    await client.post("/login", data={"username": "user_a@example.com", "password": "testpassword123"})
    ...
    async with db_session as session:
        result = await session.execute(select(Inverter).where(Inverter.id == inverter_b.id))
...
async def test_start_page_shows_only_user_inverters(client, db_session):
    ...
    user_a = await create_user_in_db(db_session, email="user_a@example.com")

    user_b = await create_user_in_db(db_session, email="user_b@example.com")

    await create_inverter_in_db(db_session, user_id=user_a.id, name="User A Inverter", serial_logger="SERIAL-A")

    await create_inverter_in_db(db_session, user_id=user_b.id, name="User B Inverter", serial_logger="SERIAL-B")
...
async def test_multiple_users_have_isolated_inverter_lists(client, db_session):
    ...
    user1 = await create_user_in_db(db_session, email="user1@example.com")
    await create_inverter_in_db(db_session, user_id=user1.id, name="User1 Inverter 1", serial_logger="U1-INV-1")
    await create_inverter_in_db(db_session, user_id=user1.id, name="User1 Inverter 2", serial_logger="U1-INV-2")

    # Create User 2 with 2 inverters
    user2 = await create_user_in_db(db_session, email="user2@example.com")
    await create_inverter_in_db(db_session, user_id=user2.id, name="User2 Inverter 1", serial_logger="U2-INV-1")
    await create_inverter_in_db(db_session, user_id=user2.id, name="User2 Inverter 2", serial_logger="U2-INV-2")

    # Login as User 1
    await client.post("/login", data={"username": "user1@example.com", "password": "testpassword123"})
    ...
    await client.post("/login", data={"username": "user2@example.com", "password": "testpassword123"})
...
async def test_database_query_filters_by_user_id(db_session):
    ...
    await create_inverter_in_db(db_session, user_id=user_a.id, serial_logger="INV-A")
    await create_inverter_in_db(db_session, user_id=user_b.id, serial_logger="INV-B")

    # Query User A's inverters
    async with db_session as session:
        result_a = await session.execute(select(Inverter).where(Inverter.user_id == user_a.id))
        ...
        result_b = await session.execute(select(Inverter).where(Inverter.user_id == user_b.id))
