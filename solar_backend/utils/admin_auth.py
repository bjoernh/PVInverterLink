...
class AdminAuth(AuthenticationBackend):
    async def login(self, request: Request) -> bool:
        ...
        async with (
            get_async_session_context() as session,  # TODO: if possible change it to fastapi dependency
        ...
        try:
            ...
            token = jwt.encode(
                {"email": user.email, "user_id": user.id, "exp": exp, "iat": datetime.now(UTC), "is_superuser": True},
    ...
    async def authenticate(self, request: Request) -> bool:
        ...
        try:
            payload = jwt.decode(token, settings.AUTH_SECRET, algorithms=["HS256"], options={"verify_exp": True})

            return bool(payload.get("is_superuser"))  # Return True for successful authentication
