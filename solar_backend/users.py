...
class UserManager(IntegerIDMixin, BaseUserManager[User, int], ModelView):
    ...
    async def validate_password(self, password: str, user: User | UserCreate) -> None:
        ...
        if len(password) < 8:
            raise InvalidPasswordException(reason="Passwort muss mindestens 8 Zeichen lang sein")
        if not any(c.isdigit() for c in password):
            raise InvalidPasswordException(reason="Passwort muss mindestens eine Zahl enthalten")
        if not any(c.isupper() for c in password):
            raise InvalidPasswordException(reason="Passwort muss mindestens einen Großbuchstaben enthalten")
    ...
    async def on_after_forgot_password(self, user: User, token: str, request: Request | None = None):
        ...
        logger.info("User has forgot their password.", user=user, token=token)
